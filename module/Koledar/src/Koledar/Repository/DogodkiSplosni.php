<?php

/*
 *  Licenca GPLv3
 */

namespace Koledar\Repository;

use Doctrine\ORM\Tools\Pagination\Paginator;
use DoctrineORMModule\Paginator\Adapter\DoctrinePaginator;
use Max\Repository\AbstractMaxRepository;

/**
 * Description of DogodkiIzven
 *
 * @author rado
 */
class DogodkiSplosni
        extends AbstractMaxRepository
{

    protected $sortOptions = [
        "default" => [
            "id" => ["alias" => "p.id"]
        ],
        "vse"     => [
            "id" => ["alias" => "p.id"]
        ]
    ];

    public function getPaginator(array $options, $name = "default")
    {
        switch ($name) {
            case "default":
                $this->expect(!empty($options['prostor']), "Prostorje obvezen", 1001130);
            case "vse":
                $qb   = $this->getVseQb($options);
                $sort = $this->getSort($name);
                $qb->orderBy($sort->order, $sort->dir);
                return new DoctrinePaginator(new Paginator($qb));
        }
    }

    public function getVseQb($options)
    {
        $qb = $this->createQueryBuilder('p');
        $e  = $qb->expr();
        if (!empty($options['q'])) {

            $naz = $e->like('p.id', ':id');

            $qb->andWhere($e->orX($naz));

            $qb->setParameter('id', "{$options['q']}%", "string");
        }
        if (!empty($options['prostor'])) {
            $qb->join('p.dogodek', 'dogodek');
            $qb->join('dogodek.prostor', 'prostor');
            $pros = $e->eq('prostor.id', ':prostor');
            $qb->andWhere($pros);
            $qb->setParameter('prostor', "{$options['prostor']}", "string");
        }
        return $qb;
    }
    /**
     * Opravila pri ustvarjanju nove entitete
     *
     * @param DogodekSplosni $object
     * @param Parameters $params
     */
    public function create($object, $params = null)
    {
        if ($object->dogodek) {
            /** @var Dogodki $rep */
            $rep = $this->getEntityManager()->getRepository('Koledar\Entity\Dogodek');
            $rep->create($object->getDogodek(), $params);
        }
        parent::create($object, $params); // TODO: Change the autogenerated stub
    }

    /**
     * Pri brisanju brišem tudi dogodek
     *
     * @param Predstava $object
     */
    public function delete($object)
    {

        parent::delete($object);
        $this->getEntityManager()->remove($object->getDogodek());
    }

}
