...
define([
    ...
    var ServerSideFilter = Backgrid.Extension.ServerSideFilter = Backbone.View.extend({
        ...
        template: _.template('<input type="search" <% if (placeholder) { %> placeholder="<%- placeholder %>" <% } %> name="<%- name %>" class="form-control" />\n\
            <a class="clear" data-backgrid-action="clear" href="#">&times;</a>', null, {variable: null}),
        /** @property */
        events: {
    ...
    var LunrFilter = Backgrid.Extension.LunrFilter = ClientSideFilter.extend({
        ...
        addToIndex: function(model) {
            ...
            if (index.documentStore.has(doc[this.ref]))
                index.update(doc);
            else
                index.add(doc);
        },
